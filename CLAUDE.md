...
- **No unbounded read.** Lists always paginated. CMS lists move to `cursorPaginate()` as
  ...
  pt-cutting, marketing, visit, transaction, transaction/order, product, customer-membership,
  customer-personal-trainer. Customer list is `simplePaginate()` + `simple_page_meta()` (its
  name sort lives on joined `users`, no cursor) — still no count. Anything walking whole table use `chunkById`/`lazyById`, never `get()` then loop.
